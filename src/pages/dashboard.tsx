...
import { useRoqPlatformUserProfiles, useSalesTargetCount, useIncentiveCount, useEmployeeCount } from 'lib/roq';

import { SalesTargetListPage as ShowCaseList } from 'pages/sales-targets';

function DashboardPage() {
  const { session } = useSession();

  const salesTargetStat = useSalesTargetCount();

  const incentiveStat = useIncentiveCount();
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Sales Targets',
      result: salesTargetStat?.data,
      isLoading: salesTargetStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Incentives',
      result: incentiveStat?.data,
      isLoading: incentiveStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Employees',
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Sales Targets
                </Text>
                <NextLink href={`sales-targets`} passHref legacyBehavior>
                  <Button
