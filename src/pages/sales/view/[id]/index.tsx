...
import { useSalesFindFirst, useRoqClient } from 'lib/roq';

import { SalesInterface } from 'interfaces/sales';
import { IncentiveListPage } from 'pages/incentives';
...
function SalesViewPage() {
  ...
  const queryParams = useMemo<{ include: { employee: boolean; company: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useSalesFindFirst(queryParams, {}, { disabled: !id });
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem
                  label="Sales Date"
                  text={data?.sales_date ? format(data?.sales_date, 'dd-MM-yyyy') : ''}
                />

                <FormListItem label="Sales Amount" text={data?.sales_amount} />

                <FormListItem label="Sales Region" text={data?.sales_region} />
            ...
            {hasAccess('incentive', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
