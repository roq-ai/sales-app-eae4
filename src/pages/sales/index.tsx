...
import { useRoqClient, useSalesFindManyWithCount } from 'lib/roq';
...
export function SalesListPage(props: SalesListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['employee', 'company', 'incentive.count'],
          searchTerm: params.searchTerm,
          searchTermKeys: ['sales_region.contains'],
          ...params.filters,
        },
        'sales',
      ),
  ...
  const { data, error, isLoading, mutate } = useSalesFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.sales.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    {
      ...
      cell: ({ row: { original: record } }: any) =>
        record?.sales_date ? format(record?.sales_date, 'dd-MM-yyyy') : '',
    },
    { id: 'sales_amount', header: 'sales_amount', accessorKey: 'sales_amount' },
    { id: 'sales_region', header: 'Sales Region', accessorKey: 'sales_region' },
