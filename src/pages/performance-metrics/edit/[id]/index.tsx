...
import { useRoqClient, usePerformanceMetricFindFirst } from 'lib/roq';
...
function PerformanceMetricEditPage() {
  ...
  const { data, error, isLoading, mutate } = usePerformanceMetricFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: PerformanceMetricInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.performance_metric.update({
        data: values as RoqTypes.performance_metric,
        where: {
          id,
        },
      });
      ...
      breadcrumbs={
        <Breadcrumbs
          items={[
            ...
            {
              label: 'Update Performance Metric',
              isCurrent: true,
            },
          ]}
        />
        ...
        <FormWrapper onSubmit={formik.handleSubmit}>
          <Flex justifyContent={'flex-start'}>
            ...
            <Button
              ...
              gap="0.5rem"
              mr="4"
              onClick={() => router.push('/performance-metrics')}
              _hover={{
                bg: 'neutral.transparent',
                color: 'neutral.main',
              }}
