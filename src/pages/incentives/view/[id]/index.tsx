...
import { useIncentiveFindFirst, useRoqClient } from 'lib/roq';

import { IncentiveInterface } from 'interfaces/incentive';

function IncentiveViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const roqClient = useRoqClient();
  const queryParams = useMemo<{ include: { employee: boolean; sales: boolean; company: boolean } }>(
    () =>
  ...
  const { data, error, isLoading } = useIncentiveFindFirst(queryParams, {}, { disabled: !id });
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('incentive', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/incentives/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem
                  ...
                  text={data?.incentive_date ? format(data?.incentive_date, 'dd-MM-yyyy') : ''}
                />

                <FormListItem label="Incentive Amount" text={data?.incentive_amount} />

                <FormListItem label="Incentive Type" text={data?.incentive_type} />
                ...
                {hasAccess('sales', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Sales"
                    text={
                      <Link as={NextLink} href={`/sales/view/${data?.sales?.id}`}>
                        {data?.sales?.sales_region + ''}
                      </Link>
                    }
                  />
                )}
