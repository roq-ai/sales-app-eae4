...
import { useRoqClient, useIncentiveFindFirst } from 'lib/roq';
...
function IncentiveEditPage() {
  ...
  const { data, error, isLoading, mutate } = useIncentiveFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: IncentiveInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.incentive.update({
        data: values as RoqTypes.incentive,
        where: {
          id,
        },
      });
